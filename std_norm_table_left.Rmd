...
x_left <- seq(-3.2, -1.5, by = 0.001)
shaded_left <- data.frame(x = x_left, y = dnorm(x_left))

ggplot(data.frame(x = c(-3.2, 3.2))) +
  geom_area(data = shaded_left, aes(x = x, y = y), fill = "darkgray") +
...
cum_probs <- round(pnorm(z_neg), input$round_digits)
...
x_left2 <- seq(-3.2, 1.5, by = 0.001)
shaded_left2 <- data.frame(x = x_left2, y = dnorm(x_left2))

ggplot(data.frame(x = c(-3.2, 3.2))) +
  geom_area(data = shaded_left2, aes(x = x, y = y), fill = "darkgray") +
  stat_function(fun = dnorm, lwd = 0.7) +
  geom_hline(yintercept = 0, lwd = 0.4) +
  geom_segment(x = 0, y = 0, xend = 0, yend = dnorm(0)/10, lwd = 0.4) +
  geom_segment(x = 1.5, y = 0, xend = 1.5, yend = dnorm(1.5), lwd = 0.4) +
...
cum_probs2 <- round(pnorm(z_pos), input$round_digits)
