...
# middle/positive z-scores
x_middle <- seq(0, 1.5, by = 0.001)
...
ggplot(data.frame(x = c(-3.2, 3.2))) +
  ...
  geom_segment(x = 0, y = 0, xend = 0, yend = dnorm(0), lwd = 0.4) +
  geom_segment(x = 1.5, y = 0, xend = 1.5, yend = dnorm(1.5), lwd = 0.4) +
  geom_segment(
    x = 2.3, y = dnorm(0)/1.4, 
    xend = 0.65, yend = dnorm(0)/2.6, 
...
# middle/positive z-scores
z_middle <- seq(0, 3.49, by = 0.01)
middle_probs <- round(pnorm(z_middle) - 0.5, input$round_digits)
table_probs <- matrix(middle_probs, nrow = length(seq(0, 3.4, 0.1)), byrow = TRUE)

first_col <- formatC(signif(seq(0, 3.4, 0.1), 2), format = "f", digits = 1)
table_probs_new <- cbind.data.frame(first_col, table_probs)

colnames(table_probs_new) <- c("z", formatC(
  signif(seq(0, 0.09, 0.01), 2), format = "f", digits = 2
))
