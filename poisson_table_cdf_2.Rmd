...
\large Table: Poisson cumulative distribution function (CDF) probabilities
\vspace{0.2in}
...
make_pois_table <- function(x_max, lambda) {
  ...
  col_names <- formatC(signif(lambda, 2), format = "f", digits = 1)
...
make_pois_table(9, seq(0.2, 2, 0.2))
```
...
```{r results = "asis"}
make_pois_table(14, seq(2.2, 4, 0.2))
```
...
\large Table: Poisson cumulative distribution function (CDF) probabilities (continued)
\vspace{0.25in}


```{r results = "asis"}
make_pois_table(18, seq(4.2, 6, 0.2))
```
...
```{r results = "asis"}
make_pois_table(21, seq(6.2, 8, 0.2))
```
...
```{r results = "asis"}
make_pois_table(24, seq(8.2, 10, 0.2))
```
